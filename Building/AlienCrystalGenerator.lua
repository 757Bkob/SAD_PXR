UndefineClass('AlienCrystalGenerator')
DefineClass.AlienCrystalGenerator = {
	__parents = { "Building", "MalfunctionOverTimeComponent", "PowerComponent", "ResConsumerDeviceComponent" },
	__generated_by_class = "ModItemBuildingCompositeDef",


	object_class = "Building",
	LockPrerequisites = {
		PlaceObj('CheckTech', {
			Tech = "AlienCrystalGenerator",
		}),
	},
	unload_anim_hands = "standing_DropDown_Hands",
	load_anim_hands = "standing_PickUp_Hands",
	BuildMenuCategory = "sub_AlienPowerPX",
	display_name = T(911971397764, --[[ModItemLootDef ScavengeGenericDebris display_name]] "Alien Crystal Generator"),
	description = T(259419572369, --[[ModItemLootDef ScavengeGenericDebris description]] "Generates an excessively large amount of <color TextEmphasis>Electricity</color><image 'UI/Icons/Resources/res_electricity' 1100> while depleting a Alien Crystal. Can produce electricity for a year. Has increased health duration and is more durable. However is prone to malfunction more frequently and requires regular maintainence. \n\n<center><style red>Contraband</style>"),
	menu_display_name = T(714348152011, --[[ModItemLootDef ScavengeGenericDebris menu_display_name]] "Alien Crystal Generator"),
	menu_description = T(639434529062, --[[ModItemLootDef ScavengeGenericDebris menu_description]] "A generator powered by Alien Crystals, I mean... what could go wrong? This has never been attempted before, and i'm assuming there is a good reason why. But I can imagine that such a majestic piece of technology would definitely power the entire planet for a long time, the only downside if ofcourse... the expensive resources needed to even obtain Alien Crystals."),
	BuildMenuIcon = "General/Gen1.png",
	BuildMenuPos = 9,
	display_name_pl = T(662722511755, --[[ModItemLootDef ScavengeGenericDebris display_name_pl]] "Alien Crystal Generator"),
	display_name_short = T(532872533474, --[[ModItemLootDef ScavengeGenericDebris display_name_short]] "Alien Generator"),
	entity = "PXBatteryGen",
	labels = {
		"PowerSources",
		"BerserkTargets",
		"SolarFlareShieldedTargets",
	},
	update_interval = 10000,
	can_turn_off = true,
	construction_cost = PlaceObj('ConstructionCost', {
		AlienCore = 2000,
		AlienCrystal = 10000,
		AlienProcessor = 2000,
		CarbonNanotubes = 200000,
	}),
	construction_points = 40000,
	repair_cost = PlaceObj('ConstructionCost', {
		CarbonNanotubes = 10000,
		Metal = 60000,
		PowerCell = 3000,
		ScrapElectronics = 10000,
	}),
	Health = 1200000,
	MaxHealth = 1200000,
	damage_reduction = {
		blunt = 30,
		energy = 30,
		gas = 30,
		pacify = 30,
		piercing = 30,
	},
	RoomPlacement = "outdoors",
	lock_block_box = box(-2000, -800, 0, 2000, 800, 700),
	MinConstructionSkill = 9,
	terrain_change = "soil",
	soil_border = 600,
	soil_form = "circle",
	apply_res_reqs = false,
	access_range = 2400,
	CustomMaterial = "Plastic",
	realtime_working_anim = true,
	attack_attraction = 20,
	affected_by_disasters = set(),
	enable_overlay_on_placement = {
		RoomsOverlay = true,
	},
	MalfunctionOverTimeComponent = true,
	PowerComponent = true,
	ResConsumerDeviceComponent = true,
	MinTimeToMalfunction = 67200000,
	MaxTimeToMalfunction = 201600000,
	MinMalfunctionDamage = 30,
	MaxMalfunctionDamage = 40,
	ChangeOwnerIcon = "UI/Icons/Infopanels/assign_owner",
	IsPowerGenerator = true,
	Production = 5000000,
	HasSmartConnection = true,
	res_consumed = "AlienCrystal",
	res_stored = 1000,
	res_operate_time = 46080000,
	res_request_threshold = 1,
	res_start_fueled = true,
}

